...
class RedBlackTreeBenchmark {
  ...
  @Setup(Level.Trial) def init: Unit = {
    ...
    val rem = scala.collection.mutable.ArrayBuffer(nums: _*)
    ...
    map1 = TreeMap(nums.map(i => (i, i)): _*)
  }

//  @Benchmark
//  def build(bh: Blackhole): Unit =
//    bh.consume(TreeSet(nums: _*))
//
//  @Benchmark
//  def buildRandom(bh: Blackhole): Unit =
//    bh.consume(TreeSet(perm: _*))
//
//  @Benchmark
...
//    var res = 0
//    for(i <- 0 to 5)
//    for(j <- 0 to 5) res += s.range(s.size*i/5, s.size*j/5).size
//    bh.consume(res)
//  }
//
//  @Benchmark
...
//    var res = 0
//    for(i <- 0 to 5)
//    for(j <- 0 to 5) res += s.slice(s.size*i/5, s.size*j/5).size
//    bh.consume(res)
//  }
//
//  @Benchmark
